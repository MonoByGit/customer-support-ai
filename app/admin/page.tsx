...
export default function AdminPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#06090e] text-slate-900 dark:text-slate-100 flex flex-col justify-between selection:bg-[#00D492] selection:text-black transition-colors">
      {/* Top Header */}
      <header className="border-b border-slate-200 dark:border-white/[0.08] bg-white/80 dark:bg-[#0b1017]/80 backdrop-blur-xl px-6 py-3.5 sticky top-0 z-30 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-3 group">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-[#00D492] to-[#075E54] flex items-center justify-center shadow-md shadow-emerald-950/20">
            <MessageSquare className="w-5 h-5 text-white" />
          </div>
          <div>
            <span className="font-bold text-base tracking-tight text-slate-900 dark:text-white group-hover:text-emerald-500 dark:group-hover:text-emerald-400 transition-colors">
              Verde AI
            </span>
            <span className="block text-[10px] text-emerald-600 dark:text-emerald-400 font-mono font-semibold">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            ...
            className="text-xs text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white bg-slate-100 dark:bg-white/[0.05] hover:bg-slate-200 dark:hover:bg-white/[0.1] border border-slate-200 dark:border-white/[0.08] px-3.5 py-1.5 rounded-xl transition-all flex items-center gap-1.5 font-medium"
          >
      ...
      <main className="max-w-4xl w-full mx-auto px-4 sm:px-6 py-12 flex-1 space-y-10">
        ...
        <div className="text-center max-w-2xl mx-auto space-y-3">
          <div className="inline-flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/25 text-emerald-700 dark:text-emerald-400 px-3.5 py-1 rounded-full text-xs font-semibold">
            <Sparkles className="w-3.5 h-3.5 text-emerald-500" />
            <span>Instant Ingest & Demo Generator</span>
          </div>
          <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-slate-900 dark:text-white">
            Genereer een WhatsApp AI Assistent
          </h1>
          <p className="text-slate-600 dark:text-slate-400 text-sm sm:text-base leading-relaxed">
            Plak een bedrijfswebsite. Ons systeem schraapt direct de diensten, prijzen en openingstijden, en maakt binnen enkele seconden een interactief WhatsApp prototype.
          </p>
        </div>

        {/* Generator Card */}
        <div className="bg-white dark:bg-[#0d141e] border border-slate-200 dark:border-white/[0.08] rounded-3xl p-6 sm:p-8 shadow-xl dark:shadow-2xl space-y-6">
          <form
            onSubmit={(e) => {
              e.preventDefault();
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <button
                ...
                className="bg-[#00D492] hover:bg-[#00be82] active:scale-95 disabled:opacity-50 text-slate-950 px-6 py-3.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 shadow-md shadow-emerald-500/20 transition-all shrink-0"
              >
          ...
          {/* Quick Presets */}
          <div className="pt-5 border-t border-slate-100 dark:border-white/[0.06] space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {presets.map((preset, idx) => (
                <button
                  ...
                  className="text-left bg-slate-50 hover:bg-slate-100 dark:bg-white/[0.03] dark:hover:bg-white/[0.06] border border-slate-200 dark:border-white/[0.06] hover:border-emerald-500/40 p-3.5 rounded-2xl transition-all group flex items-start justify-between"
                >
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 px-1.5 py-0.2 rounded font-mono uppercase font-bold">
                        {preset.badge}
                      </span>
                    </div>
                    <div className="text-[11px] text-slate-500 dark:text-slate-400">{preset.desc}</div>
                  </div>
                  <ArrowRight className="w-3.5 h-3.5 text-slate-400 group-hover:text-emerald-500 group-hover:translate-x-0.5 transition-all mt-1" />
                </button>
              ))}
            </div>
          </div>
          ...
          {status !== "idle" && (
            <div className="pt-6 border-t border-slate-100 dark:border-white/[0.08] space-y-3">
                ...
                <div
                  className={`flex items-center gap-2.5 p-2.5 rounded-xl transition-colors ${
                    status === "scraping"
                      ...
                      ? "text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-white/[0.02]"
                      : "text-slate-400"
                  }`}
                >
                  {status === "gemini" || status === "saving" || status === "done" ? (
          ...
          {status === "done" && resultProfile && (
            <div className="bg-gradient-to-br from-emerald-950 to-teal-950 border border-emerald-500/40 rounded-2xl p-6 text-white animate-fade-in space-y-4 shadow-xl">
          ...
          {/* Error Message */}
          {status === "error" && (
            <div className="bg-red-50 dark:bg-red-950/50 border border-red-200 dark:border-red-500/50 rounded-xl p-4 text-red-700 dark:text-red-200 text-xs">
      ...
      {/* Footer */}
      <footer className="border-t border-slate-200 dark:border-white/[0.08] bg-white dark:bg-[#06090e] py-6 px-6 text-center text-xs text-slate-500 dark:text-slate-400">
