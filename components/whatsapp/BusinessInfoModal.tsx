  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[85vh] flex flex-col overflow-hidden border border-gray-100 animate-scale-up">
        {/* Header */}
        <div className="bg-[#0D47A1] text-white p-5 flex items-center justify-between shrink-0">
          ...
          <div className="space-y-2 bg-gray-50 p-3.5 rounded-xl border border-gray-200/70 text-xs text-gray-700">
            ...
            {profile.websiteUrl && (
              <div className="flex items-center gap-2.5 pt-1">
                <ExternalLink className="w-4 h-4 text-[#2196F3] shrink-0" />
                <a
                  href={profile.websiteUrl}
                  target="_blank"
                  rel="noreferrer"
                  className="text-[#2196F3] hover:underline font-medium truncate"
                >
                  {profile.websiteUrl}
                </a>
              </div>
          ...
          <div>
            ...
            <div className="space-y-2">
              {profile.services.map((s) => (
                <div
                  key={s.id}
                  className="p-3 rounded-xl border border-gray-200 bg-white hover:border-[#2196F3] hover:bg-blue-50/40 transition-all flex items-center justify-between gap-3 group"
                >
                  <div>
                    ...
                    <div className="text-[11px] text-[#2196F3] font-medium mt-1">
                  ...
                  <button
                    ...
                    className="bg-[#2196F3] hover:bg-[#1E88E5] text-white text-xs font-semibold px-3 py-1.5 rounded-lg shrink-0 shadow-xs"
                  >
                    Boek via Chat
                  </button>
                </div>
